...
class SIR(base.BaseCRNModel):
    """Class for SIR network."""

    def export_to_sbml(self):
        return gillespy.export_SBML(self, "bleh.xml")
    ...
    def get_initial_settings(cls, n_settings, sigm=None):
        ...
        settings = np.random.randint(
            low=30, high=200, size=(n_settings, n_species)
        ).astype(np.float64)
        return settings
