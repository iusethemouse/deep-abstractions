...
from .deep_abstraction_manager import DeepAbstraction
...
class WorkflowManager:
    ...
    def load_simulation_data_from_memory(self, data):
    ...
    def export_deep_abstraction_weights(self):
        return self.deep_abstraction.state_dict()
    ...
    def load_deep_abstraction_weights(self, model_weights):
        self.deep_abstraction.load_state_dict(model_weights)
        self.deep_abstraction.to(device)
    ...
    def train_deep_abstraction(
        ...
        for epoch in range(n_epochs):
            if exec_context.is_canceled():
                raise RuntimeError("Node execution was cancelled, aborting training...")
    ...
    def produce_chain_of_states(
        self, n_steps=10, random_index=None, initial_state=None, timestep=0.1
    ):
        if random_index == None:
            random_index = self.get_random_index_for_initial_condition()
