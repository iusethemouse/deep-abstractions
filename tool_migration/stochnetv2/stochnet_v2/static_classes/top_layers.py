...
class RandomVariableOutputLayer(abc.ABC):
    ...
    def build_sampling_graph(self, graph=None):
        ...
        with self._sampling_graph.as_default():
            with tf.compat.v1.variable_scope(self.name_scope + "/sample"):
                ...
                self._sample_tensor = self._random_variable.sample(
                    self._sample_shape_placeholder
                )
...
class CategoricalOutputLayer(RandomVariableOutputLayer):
    ...
    def add_layer_on_top(self, base):
        with tf.compat.v1.variable_scope(self.name_scope):
            if self.hidden_size:
    ...
    def get_random_variable(self, nn_prediction_tensor):
        ...
        with tf.compat.v1.variable_scope(self.name_scope):
            with tf.compat.v1.variable_scope("random_variable"):
    ...
    def log_likelihood_function(y_true, y_pred):
        log_likelihood = -tf.nn.softmax_cross_entropy_with_logits(
    ...
    def add_layer_on_top(self, base):
        with tf.name_scope(self.name_scope) as scope:
            with tf.compat.v1.variable_scope(scope):
                ...
                mu, diag = tf.split(
                    base, num_or_size_splits=[s // 2, s - s // 2], axis=-1
                )
                ...
                if self.hidden_size:
                    with tf.compat.v1.variable_scope("residual"):
                        ...
                        with tf.compat.v1.variable_scope("diag"):
                            diag = Dense(self.hidden_size, **self._diag_layer_params)(
                                diag
                            )
                            diag = self._activation_fn(diag)
                            diag1 = Dense(self.hidden_size, **self._diag_layer_params)(
                                diag
                            )
...
class MultivariateNormalTriLOutputLayer(RandomVariableOutputLayer):
    ...
    def sample_space_dimension(self, new_sample_space_dimension):
        if new_sample_space_dimension > 1:
            ...
            self._number_of_sub_diag_entries = (
                self._sample_space_dimension * (self._sample_space_dimension - 1) // 2
            )
            self._number_of_output_neurons = (
                2 * self._sample_space_dimension + self._number_of_sub_diag_entries
            )
        else:
            raise ValueError(
    ...
    def add_layer_on_top(self, base):
        with tf.name_scope(self.name_scope) as scope:
            with tf.compat.v1.variable_scope(scope):
                ...
                mu, diag, sub_diag = tf.split(
                    base, num_or_size_splits=[s, s, shape[-1] - 2 * s], axis=-1
                )
                ...
                if self.hidden_size:
                    with tf.compat.v1.variable_scope("residual"):
                        ...
                        with tf.compat.v1.variable_scope("sub_diag"):
                            sub_diag = Dense(
                                self.hidden_size, **self._diag_layer_params
                            )(sub_diag)
                ...
                diag = tf.compat.v1.clip_by_value(diag, _DIAG_MIN, _DIAG_MAX)
                sub_diag = tf.compat.v1.clip_by_value(
                    sub_diag, -_SUB_DIAG_MAX, _SUB_DIAG_MAX
                )
    ...
    def get_random_variable(self, nn_prediction_tensor):
        def _batch_to_tril(flat_diag, flat_sub_diag):
            ...
            sub_diag_matr = tf.pad(
                sub_diag_matr, paddings=tf.constant([[0, 0], [1, 0], [0, 1]])
            )
...
class MixtureOutputLayer(RandomVariableOutputLayer):
    ...
    def set_sample_space_dimension(self):
        sample_space_dims = [
            component.sample_space_dimension for component in self.components
        ]
    ...
    def _add_layer_on_top_share(self, base):
        ...
        with tf.compat.v1.variable_scope(self.name_scope):
            ...
            components_layers = [
                component.add_layer_on_top(base) for component in self.components
            ]
    ...
    def _add_layer_on_top_individual(self, base):
        ...
        with tf.compat.v1.variable_scope(self.name_scope):
            ...
            LOGGER.debug(
                f"categorical: {self.categorical.__class__.__name__} "
            ...
            for i, component in enumerate(self.components):
                ...
                LOGGER.debug(
                    f"component {i+1}: {component.__class__.__name__} "
    ...
    def _add_layer_on_top_individual_cat(self, base):
        ...
        LOGGER.debug(
            "Mixture components share nn outputs and categorical has individual slice"
        )
        LOGGER.debug(f"base shape: {base.shape.as_list()}")
    ...
    def get_random_variable(self, nn_prediction_tensor):
        ...
        with tf.compat.v1.variable_scope(self.name_scope):
            with tf.compat.v1.variable_scope("random_variable"):
                ...
                categorical_random_variable = self.categorical.get_random_variable(
                ...
                for i, component in enumerate(self.components):
                    ...
                    component_random_variable = component.get_random_variable(
