...
def _get_mixture(config, sample_space_dimension):
    ...
    for description in descriptions:
        ...
        if description.name == "categorical":
            categorical = component_class(
                number_of_classes=len(descriptions) - 1, **kwargs
            )
        else:
            component = component_class(
                sample_space_dimension=sample_space_dimension, **kwargs
            )
    ...
    if categorical is None:
        ...
        categorical = MIXTURE_COMPONENTS_REGISTRY["categorical"](
            number_of_classes=len(descriptions)
        )
...
class StochNet:
    ...
    def _init_normal(
        ...
        if body_config_path is None:
            ...
            body_config = self._read_config(
                body_config_path, os.path.basename(self.model_explorer.body_config_fp)
            )
    ...
    def _read_config(self, file_path, model_dir_save_name=None):
        ...
        if model_dir_save_name:
            save_path = os.path.join(
                self.model_explorer.model_folder, model_dir_save_name
            )
    ...
    def _build_main_graph(self, body_fn, mixture_config):
        ...
        self.loss = self.top_layer_obj.loss_function(
            self.rv_output_ph, self.pred_tensor
        )
    ...
    def get_description(
        ...
        if nn_prediction_val is None:
            ...
            nn_prediction_val = self.session.run(
                self.pred_tensor, feed_dict={self.input_placeholder: current_state_val}
            )
    ...
    def dest_nodes(self):
        return [
            t.split(":")[0] for t in [self._sample_tensor_name, self._pred_tensor_name]
        ]
    ...
    def rescale(self, data):
        ...
        if isinstance(self.scaler, StandardScaler):
            try:
                ...
                data = (
                    data - self.scaler.mean_[: self.nb_features]
            ...
            try:
                ...
                data = (
                    data * self.scaler.scale_[: self.nb_features]
    ...
    def scale_back(self, data):
        ...
        if isinstance(self.scaler, StandardScaler):
            ...
            try:
                ...
                data = (
                    data - self.scaler.min_[: self.nb_features]
                ) / self.scaler.scale_[: self.nb_features]
        return data
    ...
    def generate_traces(
        ...
        curr_state_values = np.tile(
            curr_state_values, [n_traces, 1, 1]
        )  # (n_settings * n_traces, *state_shape)
        ...
        for step_num in tqdm(range(n_steps)):
            for n in range(n_batches):
                ...
                traces[
                    step_num + 1, n * batch_size : (n + 1) * batch_size
            ...
            if remainder:
                ...
                params = np.expand_dims(
                    traces[step_num, n_batches * batch_size :, ..., self.nb_features :],
                    0,
                )

                traces[step_num + 1, n_batches * batch_size :] = np.concatenate(
                    [next_state, params], -1
                )

            traces[step_num + 1, ..., : self.nb_features] = np.maximum(
                traces[step_num + 1, ..., : self.nb_features], zero_level
            )
