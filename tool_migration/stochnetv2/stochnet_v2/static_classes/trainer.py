...
class Trainer:
        ...
        with trainable_graph.as_default():
            ...
            if optimizer_type == "adam":
                ...
                optimizer = tf.compat.v1.train.MomentumOptimizer(
                    learning_rate, _DEFAULT_MOMENTUM
                )
            else:
    ...
    def _train(
        ...
        summary_writer = tf.compat.v1.summary.FileWriter(
            tensorboard_log_dir, session.graph
        )
        learning_rate_summary = tf.compat.v1.summary.scalar(
            "train_learning_rate", train_operations.learning_rate
        )
        ...
        train_loss_summary = tf.compat.v1.summary.scalar(
            "train_loss", tf.reduce_mean(train_operations.loss)
        )  # TODO: FOR VECTOR LOSS:

        test_mean_loss_ph = tf.compat.v1.placeholder(tf.float32, ())
        test_loss_summary = tf.compat.v1.summary.scalar(
            "test_mean_loss", test_mean_loss_ph
        )
        ...
        # cat_logits = session.graph.get_tensor_by_name(
        #     "MixtureOutputLayer/CategoricalOutputLayer/logits/BiasAdd:0"
        # )
        ...
        #     cat_logits_summaries.append(
        #         tf.compat.v1.summary.histogram("cat_logits", cat_logits[..., n])
        #     )
        #     cat_probs_summaries.append(
        #         tf.compat.v1.summary.histogram("cat_probs", cat_probs[..., n])
        #     )
        # TODO: end of TMP
        ...
        def _run_test():
            ...
            test_loss_summary_val = session.run(
                test_loss_summary, feed_dict={test_mean_loss_ph: test_mean_loss}
            )
        ...
        #         [cat_logits_summaries, cat_probs_summaries], {train_input_x: X_train}
        #     )
        ...
        def _maybe_drop_lr_tolerance(lr, tol_step, tol_best_loss):
            if learning_strategy.__class__.__name__ == "ToleranceDropLearningStrategy":
                ...
                if tol_step >= learning_strategy.epochs_tolerance:
                    lr = np.maximum(
                        learning_strategy.minimal_lr, lr * learning_strategy.lr_decay
                    )
                    tol_step = 0
        ...
        def _maybe_drop_lr_decay(lr):
            if learning_strategy.__class__.__name__ == "ExpDecayLearningStrategy":
                ...
                if learning_strategy.lr_cos_steps:
                    lr *= np.abs(
                        np.cos(
                            learning_strategy.lr_cos_phase
                            * decay_step
        ...
        def _handle_nans(counter):
            LOGGER.warning(
                f"Loss is None on step {global_step}, restore previous checkpoint..."
            )
            counter += 1
            ...
            if checkpoint_step == 0:
                ...
                try:
                    regular_checkpoints_saver.restore(
                        session, _get_regular_checkpoint_path(checkpoint_step)
                    )
                except ValueError:
                    LOGGER.info(
                    ...
                    ckpt_state = tf.compat.v1.train.get_checkpoint_state(
                        checkpoints_save_dir
                    )
                    oldest_ckpt_path = ckpt_state.all_model_checkpoint_paths[0]
                    regular_checkpoints_saver.restore(session, oldest_ckpt_path)
                _reset_optimizer()
            return counter
            ...
            for X_train, Y_train in train_dataset:
                ...
                if result["loss"] < best_loss:
                    best_loss_checkpoints_saver.save(
                        session, _get_best_checkpoint_path(global_step)
                    )
                ...
                if global_step % _REGULAR_CHECKPOINTS_DELTA == 0:
                    ...
                    regular_checkpoints_saver.save(
                        session, _get_regular_checkpoint_path(global_step)
                    )
                ...
                if learning_strategy.__class__.__name__ == "ExpDecayLearningStrategy":
                    if learning_strategy.lr_cos_steps:
                        if (
                            global_step % learning_strategy.lr_cos_steps == 0
                            and global_step > 0
                        ):
