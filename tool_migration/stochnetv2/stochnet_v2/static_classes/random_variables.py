...
class Categorical:
    def __init__(
        ...
        self.number_of_classes = self.distribution_obj._num_categories
    ...
    def get_description(self):
        ...
        with tf.Session():
            ...
            flattened_class_probabilities = tf.reshape(
                self.probs, [-1, self.number_of_classes]
            ).eval()
        ...
        for j in range(self.nb_of_independent_random_variables):
            description = (
                f"{j+1}." + description_preamble + f"  Class probabilities: \n "
    ...
    def get_description(self):
        ...
        with tf.Session():
            flattened_means = tf.reshape(
                self.mean, [-1, self.sample_space_dimension]
            ).eval()
...
class MultivariateNormalTriL:
    def __init__(self, mu, tril, validate_args=False):
...
class Mixture:
    def __init__(
        ...
        self.distribution_obj = tfd.Mixture(
            cat=tf_cat, components=tf_components, validate_args=validate_args
        )
    ...
    @property
    def description_graphkeys(self):
        description_graphkeys = {
            "cat": self.cat.description_graphkeys,
            "components": [],
        }
