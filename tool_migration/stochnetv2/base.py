    ...
    def get_histogram_bounds(cls, species_names_list=None):
        ...
        for s in species_for_histogram:
            ...
            histogram_bounds.append(
                [0.5, val * 2] if val != 0 else [0.5, cls._hist_top_bound]
            )
    ...
    def get_randomized_parameters(cls, param_names, n_settings, sigm=0.1):
        ...
        for i in range(n_settings):
            ...
            for name in param_names:
                ...
                if name not in randomized:
                    randomized[name] = []
                randomized[name].append(val)

            # Randomly choose one parameter to vary
            # param_to_vary = np.random.choice(param_names)

            # Or systematically choose each parameter based on the current setting index
            param_to_vary = param_names[i % len(param_names)]

            val = float(cls.params[param_to_vary])
            randomized[param_to_vary][-1] = np.random.uniform(
                val * (1.0 - sigm), val * (1.0 + sigm)
            )
